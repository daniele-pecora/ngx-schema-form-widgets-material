/**
 * Adapted from : https://github.com/primefaces/primeng/blob/master/src/app/components/keyfilter/keyfilter.ts
 * <br/>
 * Requires <code>strictMetadataEmit</code> to be set to <code>false</code>.<br/>
 */
...
export class KeyFilterDirective implements Validator {
    ...
    @Input('keyFilter') set pattern(_pattern: any) {
        ...
        this.regex = KeyFilterDirective.DEFAULT_MASKS[this._pattern] || this._pattern;
    }

    isNavKeyPress(e: KeyboardEvent) {
        let k = e.keyCode;
        k = DomHandler.getBrowser().safari ? (KeyFilterDirective.SAFARI_KEYS[k] || k) : k;

        return (k >= 33 && k <= 40) || k == KeyFilterDirective.KEYS.RETURN || k == KeyFilterDirective.KEYS.TAB || k == KeyFilterDirective.KEYS.ESC;
    };
    ...
    getKey(e: KeyboardEvent) {
        ...
        return DomHandler.getBrowser().safari ? (KeyFilterDirective.SAFARI_KEYS[k] || k) : k;
    }
    ...
    onKeyPress(e: KeyboardEvent) {
        ...
        if (browser.mozilla && (this.isNavKeyPress(e) || k == KeyFilterDirective.KEYS.BACKSPACE || (k == KeyFilterDirective.KEYS.DELETE && e.charCode == 0))) {
            return;
        }
